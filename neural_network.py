...
hid_neurons = 200  # neurons in hidden layer
epochs = 500
batch = 100
lr = 0.001
momentum = 0.6
...
for fold, (train, test) in enumerate(kfold.split(X, Y)):  # enumerate returns (i,values[i]) where values = kfold.split(X,Y) = (train,test)
    ...
    net = Sequential()
    net.add(Dense(hid_neurons, input_dim=N, activation='sigmoid', kernel_regularizer=l1(decay)))  # hidden layer
    net.add(Dense(M, input_dim=hid_neurons, activation='tanh', kernel_regularizer=l1(decay)))  # output layer
...
plt.title(f'Average loss with {hid_neurons} neurons in hidden layer')
plt.ylabel('Loss')
plt.xlabel('Epoch')
#plt.ylim(0,0.5)
